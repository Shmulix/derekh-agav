...
|---|---|---|---|
| `/` | `app/page.tsx` | ✅ En ligne | Home construite sur AIDA. **A** = hero (recherche Fuse.js + CTAs), verrouillé sur une hauteur d'écran (`min-h-[100svh]`, contenu centré, rythmes et taille du h1 en `clamp()` indexé sur `vh`). **I + D** = "pourquoi ce site existe", récit chronologique **centré et épuré** en trois sections, sans grille ni carte : (1) blanc, le voyage préparé au détail près puis l'angle mort de la location ("ואז מגיעים לרכב") ; (2) fond photo plein cadre en parallaxe (`components/v2/ParallaxBackdrop.tsx` + `/night-arrival.avif`, famille avec valises la nuit) : la scène "23:40" où ça se paie, plus trois causes en une ligne ; (3) blanc, "בשביל זה האתר הזה קיים" + CTA guide et articles. Tout est en texte centré + `Reveal`, aucune timeline. Suivent auteur, méthode, guide, journal, avis, CTA final. |
