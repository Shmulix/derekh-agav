...
|---|---|---|---|
| `/` | `app/page.tsx` | ✅ En ligne | Home construite sur AIDA. **A** = hero (recherche Fuse.js + CTAs), verrouillé sur une hauteur d'écran (`min-h-[100svh]`, contenu centré, rythmes et taille du h1 en `clamp()` indexé sur `vh`). **I** = trois sections enchaînées : (1) "The Problem", fil narratif en 4 temps (`journey` + `components/v2/ProblemTimeline.tsx` : route pointillée qui se trace au scroll, voiture qui roule et s'arrête avant le dernier temps, dernier segment rouge) ; (2) "Real cases", 6 scénarios concrets (`scenarios`) sur fond photo en parallaxe (`components/v2/ParallaxBackdrop.tsx` + `/counter-night.avif`), chaque carte renvoyant au chapitre ou à l'article qui traite le cas ; (3) "The Answer", la promesse en 3 temps (`answers`) + CTA guide et articles. Suivent auteur, méthode, guide, journal, avis, CTA final. |
