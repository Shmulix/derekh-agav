...
|---|---|---|---|
| `/` | `app/page.tsx` | ✅ En ligne | Home construite sur AIDA. **A** = hero (recherche Fuse.js + CTAs), verrouillé sur une hauteur d'écran (`min-h-[100svh]`, contenu centré, rythmes et taille du h1 en `clamp()` indexé sur `vh`). **I + D** = deux sections : (1) "The Journey", timeline verticale **centrée** du parcours de location en 5 étapes (`journey` + `components/v2/RentalJourney.tsx`) : jalon numéroté + titre au centre, à droite la carte "מה משתבש" (blanche, filet rouge) et à gauche la carte "מה שיש כאן" (navy, filet or, lien vers le chapitre). La route pointillée se remplit selon le scroll et les jalons s'allument au passage ; (2) bandeau final centré sur fond photo en parallaxe (`components/v2/ParallaxBackdrop.tsx` + `/counter-night.avif`) avec les CTA guide et articles. Suivent auteur, méthode, guide, journal, avis, CTA final. |
