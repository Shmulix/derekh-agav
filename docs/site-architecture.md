...
|---|---|---|---|
| `/` | `app/page.tsx` | ✅ En ligne | Home construite sur AIDA. **A** = hero (recherche Fuse.js + CTAs), verrouillé sur une hauteur d'écran (`min-h-[100svh]`, contenu centré, rythmes et taille du h1 en `clamp()` indexé sur `vh`). **I + D** = "pourquoi ce site existe", récit **centré et épuré** en quatre sections, sans grille ni carte : (1) blanc, le problème posé : "השכרת רכב היא תחום לא סלחני" ; (2) fond photo plein cadre en parallaxe (`components/v2/ParallaxBackdrop.tsx` + `/night-arrival.avif`) : la scène "23:40", le pire des cas ; (3) gris clair, les 5 règles que personne ne connaît (`facts`), chacune avec une icône maison animée (`components/v2/FactIcon.tsx`, keyframes dans `globals.css`) ; (4) blanc, le vide d'information ("אין איפה ללמוד את זה") et la raison d'être du site + CTA. Aucune timeline. Suivent auteur, méthode, guide, journal, avis, CTA final. |
